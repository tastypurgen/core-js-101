...
function getRectangleString(width, height) {
  ...
  for (let h = 1; h < height; h += 1) {
    if (h === 1) result += `┌${border}┐\n`;
    if (h === height - 1) result += `└${border}┘\n`;
    else result += `│${space}│\n`;
  }
  return result;
}
...
function encodeToRot13(str) {
  ...
  encodeToRot13.lookup = encodeToRot13.input.reduce((acc, item, i) => Object.assign(acc, { [item]: encodeToRot13.output[i] }), {});
  return str.split('').map((a) => encodeToRot13.lookup[a] || a).join('');
}
...
function isString(str) {
  if (str) return typeof str === 'string' || (typeof str === 'object' && !Array.isArray(str));
  return false;
}
